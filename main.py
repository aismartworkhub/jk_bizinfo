...
# 등록일(creatPnttm) 필터: 오늘 포함 과거 COLLECT_DAYS일 → 총 (COLLECT_DAYS+1)일치
COLLECT_DAYS = 2

# 기업마당 API: 지역당 여러 페이지(pageIndex)까지 순회 (1페이지만 보면 건수가 날짜와 무관하게 비슷하게 나올 수 있음)
BIZINFO_PAGE_UNIT = 30
BIZINFO_MAX_PAGES = 5

# Notion DB: 등록일 최신 기준 이 개수만 남기고 나머지는 아카이브(휴지통)
NOTION_MAX_ROWS = 30

notion = Client(auth=NOTION_TOKEN)


def fetch_today_announcements(collect_days=COLLECT_DAYS):
    """서울·경기 지역의 최근 N일 등록 공고 수집 (creatPnttm 기준)

    - 날짜 창: 오늘 포함 ``collect_days + 1``일 (예: collect_days=2 → 오늘·어제·그저께)
    - collect_days=1: 어제·오늘 (2일)
    - collect_days=2: 그저께·어제·오늘 (3일, main() 기본)
    - API는 areaCd와 무관하게 전국 데이터 반환 → jrsdInsttNm으로 서울/경기만 필터
    - 지역당 ``pageIndex`` 1…``BIZINFO_MAX_PAGES``까지 순회(페이지당 ``BIZINFO_PAGE_UNIT``건).
      한 페이지만 보면 그 100건 안에서만 필터되므로, ``COLLECT_DAYS``만 키워도 건수가 안 늘 수 있었음.
    """
    valid_dates = tuple(
    ...
    for area_cd, _ in [("11", "서울"), ("41", "경기")]:
        for page_index in range(1, BIZINFO_MAX_PAGES + 1):
            params = {
                "crtfcKey": BIZINFO_API_KEY,
                "dataType": "json",
                "areaCd": area_cd,
                "pageIndex": page_index,
                "pageUnit": BIZINFO_PAGE_UNIT,
            }
            try:
                ...
                if len(items) < BIZINFO_PAGE_UNIT:
                    break

            except Exception as e:
                print(f"[ERROR] API 호출 실패 (area={area_cd} page={page_index}): {e}")
                break
...
def _sort_pages_by_reg_date_desc(pages):
    """등록일 내림차순, 같은 날짜면 last_edited_time 내림차순."""
    def key(p):
        props = p.get("properties") or {}
        reg = _prop_date_start(props.get("등록일")) or "0000-00-00"
...
def trim_notion_database_keep_recent(max_rows=NOTION_MAX_ROWS):
    """등록일 기준 최신 max_rows개만 남기고, 나머지 페이지는 아카이브한다.

    Notion 테이블 보기에서 맨 위가 최신이 되게 하려면, DB 뷰 정렬도
    '등록일' 내림차순으로 한 번 맞춰 두는 것이 좋다(API로 뷰 정렬은 설정하지 않음).
    반환: (아카이브한 페이지 수, 정리 전 전체 페이지 수).
    """
    pages = query_all_notion_db_pages()
    total = len(pages)
    ordered = _sort_pages_by_reg_date_desc(pages)
    to_archive = ordered[max_rows:]
    n = 0
    for p in to_archive:
        pid = p.get("id")
        if not pid:
            continue
        try:
            notion.pages.update(pid, archived=True)
            n += 1
        except Exception as e:
            print(f"[WARN] Notion 페이지 아카이브 실패 ({pid}): {e}")
    return n, total
...
def main():
    ...
    announcements = fetch_today_announcements(COLLECT_DAYS)
    print(f"최근 {COLLECT_DAYS + 1}일(등록일 기준) 공고 {len(announcements)}건 발견")
    ...
    try:
        archived, total_before = trim_notion_database_keep_recent(NOTION_MAX_ROWS)
        if archived:
            print(f"Notion DB 정리: 등록일 최신 {NOTION_MAX_ROWS}건 유지, {archived}건 아카이브 (정리 전 {total_before}건)")
        else:
            print(f"Notion DB 정리: {total_before}건 (이하 {NOTION_MAX_ROWS}건이라 아카이브 없음)")
    except Exception as e:
        print(f"[WARN] Notion DB 행 수 정리 실패: {e}")
