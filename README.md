...
`main.py`는 실행 후 Notion DB에서 **등록일이 최신인 `NOTION_MAX_ROWS`(기본 30)개**만 남기고, 나머지 행은 **아카이브**합니다. 테이블 보기에서 위쪽이 최신이 되게 하려면 뷰 정렬을 **등록일 내림차순**으로 맞춰 두면 됩니다(API는 뷰 정렬을 바꾸지 않음).
